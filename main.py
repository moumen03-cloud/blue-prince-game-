...
CARTOGRAPHIE_IMAGES_SALLES = {
    ...
    "GUEST BEDROOM": os.path.join(DOSSIER_IMAGE, "GUESTBEDROOM.png"),
    ...
    "THE LADY CHIP'S CHAMBER": os.path.join(DOSSIER_IMAGE, "THE LADCHIP'S CHAMBER.png"),
...
PIÈCES_SPÉCIALES_DÉFINIES = {
    "VAULT", "RUMPUS ROOM", "TROPHY ROOM", "LIBRARY", "NURSERY", 
    "SECRET GARDEN", "GUEST BEDROOM", "ANTECHAMBER" # ANTECHAMBER est toujours spéciale (la fin)
}
...
class Chambre:
    def __init__(self, x, y, type_salle="standard", ressources=None, sorties=None, nom="SALLE", frais_entrée=0):
        ...
        self.cout = 3 if type_salle == "special" else 0
        self.ressources = ressources if ressources is not None else {
            "steps": 0, "coins": 0, "gems": 0, "keys": 0, "dice": 0, "tools": []
        }
...
class ApplicationPygame:
    def __init__(self, ecran):
        ...
        self.rect_ok_boite_message = None
        ...
        for nom, chemin in CARTOGRAPHIE_IMAGES_SALLES.items():
            try:
                if os.path.exists(chemin):
                    ...
                    cache_images[f"{nom}_carte"] = pygame.transform.scale(
                        img_rognee,
                        (self.TAILLE_IMAGE_SALLE_CARTE, self.TAILLE_IMAGE_SALLE_CARTE),
                    )
                    cache_images[f"{nom}_proposition"] = pygame.transform.scale(
                        img_rognee,
                        (self.TAILLE_IMAGE_PROPOSITION_CARTE, self.TAILLE_IMAGE_PROPOSITION_CARTE),
                    )
                else:
                    cache_images[f"{nom}_carte"] = self._créer_surface_substitut(
                        nom, self.TAILLE_IMAGE_SALLE_CARTE
                    )
                    cache_images[f"{nom}_proposition"] = self._créer_surface_substitut(
                        nom, self.TAILLE_IMAGE_PROPOSITION_CARTE
                    )
            except pygame.error:
                cache_images[f"{nom}_carte"] = self._créer_surface_substitut(
                    nom, self.TAILLE_IMAGE_SALLE_CARTE
    ...
    def _créer_surface_substitut(self, nom, taille, est_spéciale=False):
        ...
        for i, ligne in enumerate(texte):
            ...
            rect_texte = surface_texte.get_rect(
                center=(taille // 2, taille // 2 + i * (police_à_utiliser.get_height() + 2) - (police_à_utiliser.get_height()))
            )
    ...
    def _dessiner_carte(self):
        ...
        for y in range(self.LIGNES):
            for x in range(self.COLONNES):
                ...
                couleur_contour = (51, 51, 51)
                épaisseur_contour = 1
                if y == self.joueur.pos_y and x == self.joueur.pos_x:
                    couleur_contour = COULEUR_ACCENT
                    épaisseur_contour = 3
                ...
                if salle is not None:
                    ...
                    if img_salle is None:
                        img_salle = self._créer_surface_substitut(
                            nom_salle,
                            self.TAILLE_IMAGE_SALLE_CARTE,
                            salle.type_salle == "special",
                        )
                    ...
                    if salle.sorties.get("haut"):
                        pygame.draw.line(
                            self.ecran,
                            couleur_porte,
                            (x1 + self.CELLULE_L / 3, y1),
                            (x2 - self.CELLULE_L / 3, y1),
                            épaisseur_porte,
                        )
                    if salle.sorties.get("bas"):
                        pygame.draw.line(
                            self.ecran,
                            couleur_porte,
                            (x1 + self.CELLULE_L / 3, y2),
                            (x2 - self.CELLULE_L / 3, y2),
                            épaisseur_porte,
                        )
                    if salle.sorties.get("gauche"):
                        pygame.draw.line(
                            self.ecran,
                            couleur_porte,
                            (x1, y1 + self.CELLULE_H / 3),
                            (x1, y2 - self.CELLULE_H / 3),
                            épaisseur_porte,
                        )
                    if salle.sorties.get("droite"):
                        pygame.draw.line(
                            self.ecran,
                            couleur_porte,
                            (x2, y1 + self.CELLULE_H / 3),
                            (x2, y2 - self.CELLULE_H / 3),
                            épaisseur_porte,
                        )
    ...
    def _dessiner_panneau(self):
        ...
        inv_rect = pygame.Rect(
            self.RECT_PANNEAU.x + 24,
            self.RECT_PANNEAU.y + 16,
            (self.RECT_PANNEAU.width - 48) // 2,
            180,
        )
        ...
        res_rect = pygame.Rect(
            self.RECT_PANNEAU.x + self.RECT_PANNEAU.width // 2 + 10,
            self.RECT_PANNEAU.y + 16,
            self.RECT_PANNEAU.width // 2 - 34,
            180,
        )
        self._dessiner_texte(
            self.ecran,
        ...
        loot_rect = pygame.Rect(
            self.RECT_PANNEAU.x + 24,
            self.RECT_PANNEAU.y + 16 + 182,
            self.RECT_PANNEAU.width - 48,
            24,
        )
        self._dessiner_texte(
            self.ecran,
            f"Dernier butin : {self.dernier_butin_texte}",
            loot_rect.topleft,
            self.police_m,
            (255, 211, 105),
        )
        ...
        self._dessiner_texte(
            self.ecran,
            titre_texte,
            (self.RECT_PANNEAU.x + 24, draft_y + 10),
            self.police_g,
            titre_couleur,
        )
        ...
        if self.action == "draft":
            pygame.draw.rect(
                self.ecran,
                COULEUR_ACCENT if self.joueur.inventaire["dice"] > 0 else (60, 60, 60),
                self.RECT_REDESSINER_BOUTON,
                0,
                5,
            )
            self._dessiner_texte(
                self.ecran,
                "🎲 Redraw (D)",
                self.RECT_REDESSINER_BOUTON.center,
                self.police_m,
                COULEUR_FOND_CARTE_PRINCIPALE,
                ancre="center",
            )
            ...
            for i, r in enumerate(self.propositions):
                ...
                if img_salle is None:
                    img_salle = self._créer_surface_substitut(
                        nom_salle,
                        self.TAILLE_IMAGE_PROPOSITION_CARTE,
                        r.type_salle == "special",
                    )
                    self.images[f"{nom_salle}_proposition"] = img_salle

                rect_image = img_salle.get_rect(
                    centerx=rect_carte.centerx,
                    top=rect_carte.top + 8 + self.TAILLE_IMAGE_PROPOSITION_CARTE // 2,
                )
                self.ecran.blit(img_salle, rect_image.topleft)

                lines = [r.nom, f"TYPE: {r.type_salle.upper()}"]
                if r.type_salle == "special":
                    lines.append(f"COÛT: {r.cout}C")
                if r.frais_entrée > 0:
                    lines.append(f"FRAIS: {r.frais_entrée}C POUR ENTRER")

                rareté = CARTOGRAPHIE_RARETE.get(r.nom.upper(), 0)
                lines.append(f"RARETÉ: {rareté}")

                hauteur_ligne = self.police_m.get_height() + 2
    ...
    def _dessiner_boîte_message(self):
        ...
        self._dessiner_texte(
            self.ecran,
            titre,
            (rect_boite.centerx, rect_boite.top + 20),
            self.police_g,
            JAUNE,
            ancre="center",
        )
        self._dessiner_texte(
            self.ecran,
            texte,
            (rect_boite.centerx, rect_boite.top + 70),
            self.police_m,
            COULEUR_TEXTE,
            ancre="center",
        )
    ...
    def _définir_dernier_butin(self, gagne: dict):
        parties = []
        for k in ("steps", "coins", "gems", "keys", "dice"):
            if k in gagne and gagne[k] != 0:
                v = gagne[k]
                signe = "+" if v > 0 else ""
                parties.append(f"{k}{signe}{v}")
        if "tools" in gagne and gagne["tools"]:
    ...
    def _gérer_sélection_salle_draft(self, index):
        ...
        if salle_choisie.type_salle == "special" and not self.joueur.payer("coins", 3):
            self.boite_message = (
                "Pièce Spéciale",
                "Pas assez de Pièces (3 Pièce). Annulation du placement.",
            )
        ...
        if salle_choisie.frais_entrée > 0:
            if not self.joueur.payer("coins", salle_choisie.frais_entrée):
                self.boite_message = (
                    "Salle Payante",
                    f"Besoin de {salle_choisie.frais_entrée} pièces pour entrer. Placement annulé.",
                )
                self.joueur.pas_restants += 1
                self.action = None
                return
    ...
    def _gérer_mouvement(self, cle):
        ...
        d = {"haut": (-1, 0), "bas": (1, 0), "gauche": (0, -1), "droite": (0, 1)}[cle]
        dy, dx = d
        ny, nx = py + dy, px + dx
        ...
        if self.donjon[ny][nx] is None:
            if salle_actuelle and salle_actuelle.sorties.get(cle, False):
                ...
                self.boite_message = (
                    "Bloqué",
                    "Impossible de construire une pièce ici (pas de sortie dans la pièce actuelle).",
                )
        else:
    ...
    def gérer_événement(self, événement):
        if self.boite_message:
            if (
                événement.type == pygame.MOUSEBUTTONDOWN
                and événement.button == 1
                and self.rect_ok_boite_message is not None
                and self.rect_ok_boite_message.collidepoint(événement.pos)
            ):
                self.boite_message = None
            if événement.type == pygame.KEYDOWN and événement.key == pygame.K_RETURN:
                self.boite_message = None
        ...
        if événement.type == pygame.KEYDOWN:
            if événement.key == pygame.K_UP:
                self._gérer_mouvement("haut")
            elif événement.key == pygame.K_DOWN:
                self._gérer_mouvement("bas")
            elif événement.key == pygame.K_LEFT:
                self._gérer_mouvement("gauche")
            elif événement.key == pygame.K_RIGHT:
                self._gérer_mouvement("droite")
            elif self.action == "draft":
                if événement.key == pygame.K_1:
                    self._gérer_sélection_salle_draft(0)
                elif événement.key == pygame.K_2:
                    self._gérer_sélection_salle_draft(1)
                elif événement.key == pygame.K_3:
                    self._gérer_sélection_salle_draft(2)
                elif événement.key == pygame.K_d:
                    self._gérer_redessiner()

        elif événement.type == pygame.MOUSEBUTTONDOWN and événement.button == 1:
            if (
                self.RECT_REDESSINER_BOUTON.collidepoint(événement.pos)
                and self.action == "draft"
                and self.joueur.inventaire["dice"] > 0
            ):
                self._gérer_redessiner()
            elif self.action == "draft":
